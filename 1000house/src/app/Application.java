...
public class Application implements Serializable {
	...
	/**
	 * Sets the name of the application and if it's the first time that someone
	 * creates that app, it creates the user list (readed from file) and the admin
	 * user. Otherwise, reads the saved app and restores the values it had the last
	 * time someone created it.
	 * 
	 * @param name
	 */
	...
	/**
	 * Search in the app offer list the ones where the house has the code given as
	 * zip code
	 * 
	 * @param code
	 *            of the house
	 ...
	 * @throws NotRegisteredUser
	 *             if the user who tries to search is not registered
	 */
	 ...
	 * 
	 * @param rate
	 * @return a list of avoffers with an average rate equal or higher than the rate
	 *         given
	 * @throws NotRegisteredUser
	 *             if the user who tries to search is not registered
	 */
	...
	/**
	 * Checks that the offers asked for changes of the user given are on date to be
	 * changed. If it isn't on date,it removes it from the waiting-for-review app
	 * list and set it state to DENIED
	 * 
	 * @param u
	 *            : the user
	 * @param app
	 *            : app were the user is trying to log
	 * @return true
	 * @throws NotHost
	 */
				...
				if (o.getState().equals(OfferStates.CHANGES)) {
					if (o.getChangesDate().getYear() == ModifiableDate.getModifiableDate().getYear()) {
						if (o.getChangesDate().getMonthValue() == ModifiableDate.getModifiableDate().getMonthValue()) {
							if ((ModifiableDate.getModifiableDate().getDayOfMonth()
									- o.getChangesDate().getDayOfMonth()) > 5) {
								app.getwaitoffers().remove(o);
								o.setState(OfferStates.DENIED);
								return true;
							}
						} else if (o.getChangesDate().getMonthValue() < ModifiableDate.getModifiableDate()
								.getMonthValue()) {
							app.getwaitoffers().remove(o);
							o.setState(OfferStates.DENIED);
							return true;
	...
	/**
	 * Checks that the user's reserves are still on date to be bought. If it isn't
	 * on date,it removes it from the user-reserves list
	 * 
	 * @param u
	 * @return
	 * @throws NotGuest
	...
	/**
	 * Logs the user with the password and NIF given in the app. Also checks that
	 * the offers asked for changes and the reserves are still on date to be
	 * changed(offers9 or paid(reserves) Then logs the user in
	 * 
	 * @param NIF
	 *            of the user
	 * @param password
	 ...
	 * @throws InvalidNIF
	 *             if there is an user in the file with the same NIF that another
	 *             user
	 * @throws NotRegisteredUser
	 *             if a user that it's not in the users list tries to log in
	 */
	...
	public Boolean logout() {
		try {
			...
			if (log.getState().equals(UserStates.BANNED) == false)
	...
	/**
	 * Creates a Living offer, adding it to the waiting for review list of the app
	 * and to the house offers list
	 * 
	 * @param house
	 *            :the house of the offer
	 * @param iniDate
	 *            : date when the offer begins
	 * @param numMonths
	 *            : number of months of the offer
	 * @param price
	 *            :price per month of the offer
	 * @return true if everything is correct, false otherwise
	 * @throws NotHost
	 *             if an user who is not a host tries to create an offer
	 * @throws NotOwner
	 *             if a host tries to create an offer on a house that doesn't belong
	 *             to him
	...
	/**
	 * Creates a Holiday offer, adding it to the waiting for review list of the app
	 * and to the house offers list
	 * 
	 * @param house
	 *            :the house of the offer
	 ...
	 *             if the house already has an offer of that type
	 */
	public Boolean createOffer(House house, LocalDate iniDate, LocalDate endDate, int price, int deposit)
			throws Exception {
		try {
			if (log.getState() != UserStates.CONNECTED_HOST) {
				throw new NotHost();
